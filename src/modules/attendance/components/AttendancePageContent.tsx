...
export default function AttendancePageContent() {
  ...
  const canGate = isAdmin || user?.role === UserRole.ATTENDANCE_OPERATOR;
  const [tab, setTab] = useState<Tab>(canGate ? "gate" : "class");
  ...
  const handleCameraScan = useCallback((payload: string) => { setQrPayload(payload); setNotice("QR leído con cámara. Valídalo para continuar."); }, []);
  ...
  const refresh = useCallback(async () => {
    ...
    try { await Promise.all([canGate ? loadGate() : Promise.resolve(), loadAcademic()]); }
  ...
  async function createPickupContact(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const response = await api.post("/attendance/pickup-contacts", { studentProfileId: pickupStudent.studentProfileId, name: form.get("name"), relationship: form.get("relationship"), phone: form.get("phone") || undefined, validFrom: form.get("validFrom") || undefined, validUntil: form.get("validUntil") || undefined, requiresIdCheck: true });
      const photo = form.get("photo");
      if (photo instanceof File && photo.size) { const upload = new FormData(); upload.append("file", photo); await api.post(`/attendance/pickup-contacts/${response.data.data.id}/photo`, upload); }
  ...
  async function deactivatePickupContact(id: string) {
    if (!pickupStudent) return;
    setBusy(true); setError(null);
    try { await api.patch(`/attendance/pickup-contacts/${id}`, { active: false }); await openPickupContacts(pickupStudent); setNotice("La autorización fue desactivada."); }
    catch (requestError) { showRequestError(requestError, "No fue posible desactivar la autorización."); }
    finally { setBusy(false); }
  }

  async function selectPickupContact(id: string) {
    setPickupContactId(id); setPickupPhotoUrl(null);
    if (!id) return;
    try { const response = await api.get(`/attendance/pickup-contacts/${id}/photo`); setPickupPhotoUrl(response.data.data?.url ?? null); }
    catch { setPickupPhotoUrl(null); }
  }
...
function PickupContactsModal({ student, contacts, busy, onClose, onSubmit, onDeactivate }: { student: Student; contacts: PickupContact[]; busy: boolean; onClose: () => void; onSubmit: (event: FormEvent<HTMLFormElement>) => void; onDeactivate: (id: string) => void }) { return <div className="fixed inset-0 z-50 grid place-items-center bg-black/70 p-4"><div className="glass-panel w-full max-w-lg rounded-2xl p-6 space-y-5"><div className="flex items-start justify-between gap-4"><div><h2 className="font-bold text-xl">Personas autorizadas</h2><p className="text-sm text-[var(--text-secondary)]">{student.firstName} {student.lastName}. Portería siempre confirma identificación visual.</p></div><button onClick={onClose} className="glass-button-secondary !px-3 !py-2">Cerrar</button></div><div className="max-h-40 overflow-y-auto divide-y divide-[var(--border-glass)] rounded-xl border border-[var(--border-glass)]">{contacts.map((contact) => <div className="flex items-center justify-between gap-3 p-3 text-sm" key={contact.id}><div><p className="font-medium">{contact.name}</p><p className="text-[var(--text-secondary)]">{contact.relationship}{contact.photoPath ? " · Foto registrada" : ""}</p></div>{contact.active !== false && <button type="button" disabled={busy} onClick={() => onDeactivate(contact.id)} className="text-xs text-[var(--danger)]">Desactivar</button>}</div>)}{!contacts.length && <p className="p-4 text-sm text-[var(--text-secondary)]">Aún no hay personas autorizadas.</p>}</div><form onSubmit={onSubmit} className="grid gap-3 md:grid-cols-2"><input required name="name" className="glass-input" placeholder="Nombre completo" /><input required name="relationship" className="glass-input" placeholder="Parentesco" /><input name="phone" className="glass-input" placeholder="Teléfono (opcional)" /><label className="space-y-1 text-sm"><span>Vigencia desde <span className="text-[var(--text-muted)]">(opcional)</span></span><input name="validFrom" type="date" className="glass-input w-full" /></label><label className="space-y-1 text-sm"><span>Vigencia hasta <span className="text-[var(--text-muted)]">(opcional)</span></span><input name="validUntil" type="date" className="glass-input w-full" /></label><p className="text-xs text-[var(--text-secondary)] md:col-span-2">Déjalas vacías para una autorización sin fecha de vencimiento. Úsalas para permisos temporales.</p><label className="text-xs text-[var(--text-secondary)] md:col-span-2">Foto opcional (JPG, PNG o WebP; máximo 2 MB). No se almacena identificación oficial.<input name="photo" type="file" accept="image/jpeg,image/png,image/webp" className="mt-1 block w-full text-sm" /></label><button disabled={busy} className="glass-button justify-center md:col-span-2">Agregar persona autorizada</button></form></div></div>; }
