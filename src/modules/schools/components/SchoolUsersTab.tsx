...
function UserRow({ user, actionLoadingId, t, onEdit, onToggleActive }: { user: SchoolUser; actionLoadingId: string | null; t: typeof translations.es; onEdit: SchoolUsersTabProps["onEdit"]; onToggleActive: SchoolUsersTabProps["onToggleActive"] }) {
  const role = user.role === UserRole.SUPER_ADMIN ? t.schools.users.roles.SUPER_ADMIN : user.role === UserRole.TEACHER ? t.schools.users.roles.TEACHER : user.role === UserRole.ATTENDANCE_OPERATOR ? "Operador de asistencias" : t.schools.users.roles.SCHOOL_ADMIN;
