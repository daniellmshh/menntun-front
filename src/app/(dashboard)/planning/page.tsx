...
export default function PlanningListPage() {
  ...
  const handleDeleteClick = (e: React.MouseEvent, planning: Planning) => {
  ...
  const confirmDelete = async () => {
    ...
    try {
      const res = await deletePlanning(deleteTarget.id);
      if (res.error) {
        alert(res.error);
      } else {
        setPlannings((prev) => prev.filter((p) => p.id !== deleteTarget.id));
    ...
    <ModuleGuard moduleKey="planning" requireSchoolContext={false}>
                ...
                >
                  ...
                  <div className="flex justify-between items-center pt-4 border-t border-white/5 mt-4">
                    ...
                    <div className="flex items-center gap-2">
                      <button
                        onClick={(e) => handleDeleteClick(e, planning)}
                        className="p-2 rounded-lg bg-[var(--bg-base)] border border-[var(--border-glass)] text-[var(--text-muted)] hover:text-red-500 hover:border-red-500/30 hover:bg-red-500/10 transition-all opacity-0 group-hover:opacity-100"
      ...
      {mounted && deleteTarget && createPortal(
        <ConfirmDeleteModal
          ...
          description={
            language === "es"
              ? <>¿Estás seguro de eliminar la planeación <strong className="text-[var(--text-primary)]">"{deleteTarget.title}"</strong>?</>
              : <>Are you sure you want to delete the lesson plan <strong className="text-[var(--text-primary)]">"{deleteTarget.title}"</strong>?</>
          }
